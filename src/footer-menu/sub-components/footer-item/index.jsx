    ...
    <div className={`oo-fm-item ${isOpen ? "is-open" : ""}`}>
      {href ? (
        <h6
          ...
          className={`oo-fm-item-heading oo-fm-item-heading--link ${locale}`}
        >
          <Link href={href}>{heading}</Link>
        </h6>
      ) : (
        <h6 onClick={onHandleClick} className={`oo-fm-item-heading ${locale}`}>
          {heading}
        </h6>
      )}
      <div ref={content} className="oo-fm-items-group">
        {children}
      </div>
    </div>
  );
